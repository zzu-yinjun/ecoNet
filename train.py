...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.mode.mode != "train":
        raise Exception(
            "This script is for training only. Please run test.py for evaluation"
        )
    ...
    if cfg.dataset.name == "batvisionv1":
        train_set = AudioVisualDataset("mp3d", "train")
        if cfg.mode.validation:
            val_set = AudioVisualDataset("mp3d", "val")
    elif cfg.dataset.name == "replica":
        print("replica")
        train_set = AudioVisualDataset("replica", "train")
        if cfg.mode.validation:
            val_set = AudioVisualDataset("replica", "val")
    ...
    # model= UNet_aspp(in_channels=2, num_classes=1, base_c=64)
        ...
        if cfg.mode.validation and epoch % cfg.mode.validation_iter == 0:
            ...
            with torch.no_grad():
                for data in val_loader:
                    audio_spec_val = data["audio_spec"].to(device)
                ...
                if mean_errors[1] < best_rmse:
                    best_checkpoints = epoch
                    best_rmse = mean_errors[1]
                    print("bestcheckpoints:" + str(epoch))

                print("RMSE: {:.3f}".format(mean_errors[1]))
                print("detail1: {:.3f}".format(mean_errors[2]))
                val_errors = {}
                val_errors["ABS_REL"], val_errors["RMSE"] = (
                    mean_errors[0],
                    mean_errors[1],
                )
                ...
                if epoch % cfg.mode.validation_display == 0:
                    tensorboard_display_input_pred(
                        writer, audio_spec_val, depth_pred_val, gtdepth_val, "Val", epoch
                    )

        # ------- Display tensorboard ------------
        if epoch % cfg.mode.print_tensorboard == 0:
            tensorboard_display_input_pred(
                writer, audio_spec, depth_pred, gtdepth, "Train", epoch
            )
